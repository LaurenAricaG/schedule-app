...
import { FiCheckCircle, FiCircle, FiEdit2, FiTrash2, FiClock, FiBell, FiAlertCircle, FiClipboard, FiCalendar, FiEye } from "react-icons/fi";
...
export function TaskItem({ task, onEdit, isAdminView = false }: TaskItemProps) {
  ...
  const handleToggle = () => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        toast.success(`Tarea ${isCompleted ? "marcada como pendiente" : "completada"}`);
      } else {
    ...
    >
      ...
      <div className="flex items-center gap-6 min-w-0">
        <div className={cn(
          ...
          isCompleted ? "bg-foreground/5 text-foreground/40" : "bg-primary/10 text-primary shadow-sm"
        )}>
          <FiClipboard size={28} />
        </div>
        ...
        <div className="grow min-w-0 flex flex-col gap-1 text-left">
          <div className="flex items-center gap-2 mb-1">
            <span className={cn(
              ...
              isCompleted ? "bg-foreground/5 text-foreground/40" : "bg-primary/10 text-primary"
            )}>
              Actividad
            </span>
            {isOverdue && (
          ...
          <h3 className={cn(
            ...
            isCompleted ? "line-through text-foreground-muted/60" : "text-foreground"
          )}>
            {task.title}
          </h3>
